...
function buildProductMaterialSelection(product, materials = []) {
  const fromSelection = normalizeMaterialSelection(product?.materialSelection || product?.defaults?.materialSelection || {});
  ...
  return buildSelectionFromLegacy(
    product?.materialId || "",
    product?.laminateMaterialId || product?.defaults?.laminateMaterialId || "",
    materials
  );
}
...
export default function ProductsPage() {
  ...
  const openCreate = () => {
    ...
    setForm({
      ...
      materialId: "",
      laminateMaterialId: "",
      materialSelection: normalizeMaterialSelection(),
    });
    setDialogOpen(true);
  };
  ...
  const openEdit = (p) => {
    ...
    setForm({
      ...
      materialSelection: buildProductMaterialSelection(p, materials),
  ...
  const save = async () => {
    ...
    if (form.calculatorType === "manual_item") {
      ...
      payload.materialSelection = normalizeMaterialSelection();
      payload.materialExtras = [];
      payload.defaults = {};
    } else {
      const savedSelection = buildMaterialSelectionForSave(form.materialSelection || {});
      const rawBaseMaterial = materials.find((m) => m.id === savedSelection.primaryMaterialId) || null;
      const rawLaminateMaterial = materials.find((m) => m.id === savedSelection.laminateMaterialId) || null;

      payload.unitPrice = 0;
      payload.materialId = savedSelection.primaryMaterialId || "";
      payload.laminateMaterialId = savedSelection.laminateMaterialId || "";
      payload.materialSelection = savedSelection;
      payload.materialExtras = savedSelection.extras || [];
  ...
  useEffect(() => {
    if (form.calculatorType === "sheet_sign_manual_yield") {
      ...
      if (!form.materialId || !sheetBaseMaterials.some((m) => m.id === bm?.id)) {
        setForm((p) => ({
          ...p,
          materialId: sheetBaseMaterials[0]?.id || "",
          materialSelection: {
            ...normalizeMaterialSelection(p.materialSelection),
            primaryType: "sheet_media",
            primaryMaterialId: sheetBaseMaterials[0]?.id || "",
          },
        }));
      }
    ...
    if (form.calculatorType === "roll_print_by_metre") {
      ...
      if (!form.materialId || !rollBaseMaterials.some((m) => m.id === bm?.id)) {
        setForm((p) => ({
          ...p,
          materialId: rollBaseMaterials[0]?.id || "",
          materialSelection: {
            ...normalizeMaterialSelection(p.materialSelection),
            primaryType: "roll_media",
            primaryMaterialId: rollBaseMaterials[0]?.id || "",
          },
        }));
      }
  ...
  const preview = useMemo(() => {
    ...
    if (form.calculatorType === "manual_item") {
      basePreview = calcManualItem({ qty: previewQty, unitPrice: form.unitPrice, discountPct: qd, rateCard });
    } else if (form.calculatorType === "sheet_sign_manual_yield") {
    ...
    const pricedExtras = priceMaterialExtras(form.materialSelection?.extras || [], materials, {
      lineQty: previewQty,
      discountPct: qd,
    });

    return applyExtrasToPreview(basePreview, pricedExtras, { lineQty: previewQty });
            ...
            {form.calculatorType === "manual_item" ? (
              ...
              <>
                <MaterialSelectorSection
                  ...
                  materials={materials}
                  jobProfile={getProductMaterialProfileFromCalculator(form.calculatorType)}
                  value={form.materialSelection}
                  onChange={(nextSelection) =>
                    setForm((p) => ({
                      ...p,
                      materialSelection: nextSelection,
                      materialId: nextSelection.primaryMaterialId || "",
                      laminateMaterialId: nextSelection.laminateMaterialId || "",
                    }))
                  }
                />
              ...
              {!preview ? (
                ...
                <>
                  ...
                  <Typography sx={{ opacity: 0.75, mt: 0.5 }}>
                    Stock: ${Number(preview.breakdown.stockTotal ?? preview.breakdown.materialTotal ?? 0).toFixed(2)} • Lam: ${Number(preview.breakdown.laminateTotal ?? 0).toFixed(2)} • Ink: ${Number(preview.breakdown.inkTotal ?? 0).toFixed(2)} • Labour: ${Number(preview.breakdown.labourTotal ?? 0).toFixed(2)} • Extras: ${Number(preview.breakdown.extrasSellTotal ?? 0).toFixed(2)}
                  </Typography>
                </>
              )}
            </Paper>
