...
export function getMaterialApplicableTo(material) {
  const direct = Array.isArray(material?.applicableTo)
    ? material.applicableTo
        .map((item) => normalizeMaterialType(item))
        .filter(Boolean)
    : [];
...
export function supportsRollBase(material) {
  ...
  if (type === "roll_media") {
    return (
      (hasRollCost(material) || getMaterialCostPerUnit(material) > 0) &&
      widthMm > 0
    );
  }
...
function laminateAppliesToPrimaryType(laminate, primaryTypeInput) {
  ...
  if (isSheetFamilyType(primaryType) && applicableTo.includes("sheet_media")) {
    return true;
  }
  return false;
}
...
export function getLaminateOptions(materials = [], primaryMaterialOrType) {
  return materials.filter((material) =>
    supportsLaminateForBase(material, primaryMaterialOrType)
  );
}
