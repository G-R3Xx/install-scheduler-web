...
export default function QuoteEditPage() {
  ...
  const openAddLine = () => {
    ...
    setLiLaminateMaterialId(first?.laminateMaterialId || d.laminateMaterialId || "");
    ...
    if ((first?.calculatorType || "manual_item") === "manual_item") {
      ...
      setLiMaterialId(first?.materialId || rollBaseMaterials[0]?.id || "");
      ...
      setLiMaterialId(first?.materialId || sheetBaseMaterials[0]?.id || "");
  ...
  const openEditLine = (li) => {
    ...
    setLiMaterialId(inputs.materialId || "");
    setLiLaminateMaterialId(inputs.laminateMaterialId || "");
  ...
  useEffect(() => {
    ...
    if (ct === "sheet_sign_manual_yield") {
      ...
      if (!liMaterialId || !sheetBaseMaterials.some((m) => m.id === bm?.id)) {
        setLiMaterialId(selectedProduct.materialId || sheetBaseMaterials[0]?.id || "");
      }
    ...
    if (ct === "roll_print_by_metre") {
      ...
      if (!liMaterialId || !rollBaseMaterials.some((m) => m.id === bm?.id)) {
        setLiMaterialId(selectedProduct.materialId || rollBaseMaterials[0]?.id || "");
      }
      const lm = materials.find((m) => m.id === liLaminateMaterialId);
  ...
  const linePreview = useMemo(() => {
    ...
    if (ct === "manual_item") {
      return calcManualItem({ qty: q, unitPrice: num(liUnitPrice, 0), discountPct: combinedDiscountPct, rateCard });
    }
            ...
            {selectedProduct?.calculatorType === "manual_item" ? (
              ...
              <>
                <FormControl fullWidth>
                  ...
                  <Select label="Base Material" value={liMaterialId} onChange={(e) => setLiMaterialId(e.target.value)}>
                    {(selectedProduct?.calculatorType === "roll_print_by_metre" ? rollBaseMaterials : sheetBaseMaterials).map((m) => <MenuItem key={m.id} value={m.id}>{getMaterialDisplayName(m)}</MenuItem>)}
                  </Select>
                </FormControl>

                <FormControl fullWidth>
                  <InputLabel>Laminate (optional)</InputLabel>
                  <Select label="Laminate (optional)" value={liLaminateMaterialId} onChange={(e) => setLiLaminateMaterialId(e.target.value)}>
                    <MenuItem value=""><em>None</em></MenuItem>
                    {(selectedProduct?.calculatorType === "roll_print_by_metre" ? rollLaminateOptions : sheetLaminateOptions).map((m) => <MenuItem key={m.id} value={m.id}>{getMaterialDisplayName(m)}</MenuItem>)}
                  </Select>
                </FormControl>
              ...
              {!linePreview ? (
                ...
                <>
                  ...
                  <Typography sx={{ opacity: 0.8, mt: 0.5 }}>
                    Stock: {money(linePreview.breakdown.stockTotal ?? linePreview.breakdown.materialTotal ?? 0)} • Lam: {money(linePreview.breakdown.laminateTotal ?? 0)} • Ink: {money(linePreview.breakdown.inkTotal ?? 0)} • Labour: {money(linePreview.breakdown.labourTotal ?? 0)}
                  </Typography>
                </>
              )}
            </Paper>
