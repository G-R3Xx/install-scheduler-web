    ...
    if (isDrawing && tool === 'rect') {
      setShapes((arr) =>
        arr.map((s) => {
          ...
          // Only prompt if it's a meaningful rectangle (not a click)
          if (Math.abs(s.width) < 2 && Math.abs(s.height) < 2) return s;
          const current = s.text || '';
          const label = prompt('Enter text for this box (optional):', current);
    ...
    return (
      <>
        ...
        <Group
          ...
          y={mid.y}
          onClick={() => {
            const v = prompt('Enter measurement (mm):', s.measure != null ? String(s.measure) : '');
            if (v == null) return;
            const n = Number(v);
            setShapes((arr) =>
      ...
      {imgLoaded && bgImage && (
        ...
        >
          <Layer>
            ...
            {shapes.map((s) => {
              if (s.type === 'rect') {
                // Support negative width/height by normalizing draw order for text centering
