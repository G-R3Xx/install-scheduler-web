...
export default function OrderTasksPanel({ orderId, order, highlightTaskTitle = "" }) {
  ...
  const generateTasks = async () => {
    ...
    if (!Array.isArray(order?.lineItems) || !order.lineItems.length) {
      setSnack({
        open: true,
        msg: "This order has no line items to generate tasks from.",
        severity: "warning",
      });
      return;
    }
    ...
    try {
      ...
      if (!existing.empty) {
        setSnack({
          open: true,
          msg: "Tasks already exist for this order.",
          severity: "info",
        });
        setGenerating(false);
        return;
  ...
  return (
    <Paper sx={{ p: 2, borderRadius: 3 }}>
        ...
        {tasks.length === 0 ? (
          ...
          <Button
            ...
            onClick={() =>
              setSnack({
                open: true,
                msg: "Tasks already exist. Delete them manually first if you want to rebuild.",
                severity: "info",
              })
            }
      ...
      {loading ? (
        ...
        <Alert severity="info">
          No production tasks yet. New orders can generate tasks automatically from the quote, or you can click{" "}
          <strong>Generate Tasks</strong>.
        </Alert>
      ) : (
        <Stack spacing={2}>
            ...
            <Paper key={groupName} variant="outlined" sx={{ p: 1.5, borderRadius: 2 }}>
              ...
              <Stack spacing={1}>
                {groupTasks.map((task) => {
                  const done = task.status === "done";
                  const isHighlighted =
                    normalizeTitle(task?.title) === normalizeTitle(highlightTaskTitle) &&
                    task?.status !== "done";
                      ...
                      <Stack direction="row" spacing={1.25} alignItems="flex-start">
                        ...
                        <Box sx={{ flex: 1 }}>
                          <Stack direction="row" spacing={1} alignItems="center" useFlexGap flexWrap="wrap">
                            ...
                            {task.productName ? (
                              <Chip size="small" variant="outlined" label={task.productName} />
                            ) : null}
                          </Stack>

                          {task.description ? (
                            <Typography sx={{ opacity: 0.8, mt: 0.5 }}>{task.description}</Typography>
                          ) : null}
                          ...
                          {done ? (
                            <Typography sx={{ opacity: 0.65, fontSize: 12, mt: 0.75 }}>
                              Completed by <strong>{task.completedByName || "Unknown"}</strong> •{" "}
                              {formatTs(task.completedAt)}
                            </Typography>
                          ) : null}
                        </Box>
                      </Stack>
